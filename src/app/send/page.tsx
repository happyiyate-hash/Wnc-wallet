  ...
  const handleSendRequest = async () => {
    ...
    try {
      if (viewingNetwork.type === 'xrp') {
        ...
        // Polkadot Decimals are usually 10 for DOT
      ...
      <main className="flex-1 overflow-hidden">
        <ScrollArea className="h-full">
          <div className="p-6 space-y-8 pb-40">
            <div className="flex flex-col items-center">
              <button onClick={() => setIsNetworkSheetOpen(true)} className="flex items-center gap-3 p-2 rounded-[2rem] bg-primary/10 border border-primary/20 hover:bg-primary/20 transition-all group shadow-xl active:scale-95">
                <div className="p-1">
                  <TokenLogoDynamic logoUrl={selectedToken?.iconUrl} alt={selectedToken?.name || ''} size={36} chainId={selectedToken?.chainId} name={selectedToken?.name} symbol={selectedToken?.symbol}/>
                </div>
                <div className="text-left pr-4">
                  <h2 className="text-sm font-black text-white uppercase tracking-tight">{selectedToken?.symbol || 'Select Asset'}</h2>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="bg-primary/5 border border-primary/10 rounded-[2rem] p-2 backdrop-blur-xl">
                  <Input 
                    placeholder={viewingNetwork.type === 'xrp' ? "rAddress..." : viewingNetwork.type === 'polkadot' ? "SS58 Address..." : "0x... or ENS"} 
                    value={recipient} 
              ...
              <div className="space-y-3">
                ...
                <div className="bg-primary/5 border border-primary/10 rounded-[2.5rem] p-6 backdrop-blur-xl relative group">
                  <div className="flex items-baseline justify-between gap-4">
                    ...
                    />
                    <Button size="sm" variant="ghost" className="text-primary font-black hover:bg-primary/10 rounded-xl text-[10px] uppercase tracking-widest h-10 px-4" onClick={() => setAmount(balance.toString())}>MAX</Button>
                  </div>
                  <div className="mt-2 text-xs font-bold text-muted-foreground/40 italic">≈ ${amountUsdValue.toLocaleString('en-US', { minimumFractionDigits: 2 })} USD</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background via-background/95 to-transparent backdrop-blur-md z-40">
          <div className="max-w-md mx-auto">
            ...
            <Button 
              className={cn("w-full h-16 rounded-[2rem] text-lg font-black shadow-2xl transition-all duration-300 border-b-4", canSend ? "bg-primary hover:bg-primary/90 border-primary/50 shadow-primary/30" : "bg-zinc-800 border-zinc-900 opacity-50 grayscale cursor-not-allowed text-zinc-500 shadow-none")} 
              disabled={!canSend} 
          ...
          <div className="flex flex-col h-full relative z-10">
            ...
            <ScrollArea className="flex-1 px-6">
              <div className="grid grid-cols-2 gap-3 pb-24">
                {allChains.map((chain) => (
                  <button key={chain.chainId} onClick={() => { setSelectedNetworkForSelection(chain); setIsTokenSideSheetOpen(true); }} style={{ borderColor: `${chain.themeColor || '#818cf8'}40`, background: `linear-gradient(135deg, ${chain.themeColor || '#818cf8'}15 0%, rgba(0,0,0,0) 100%)` }} className="flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all text-center">
          ...
          <ScrollArea className="flex-1 p-4">
            <div className="space-y-2 pb-20">
              {selectedNetworkForSelection && getAvailableAssetsForChain(selectedNetworkForSelection.chainId).map((token) => { 
                ...
                return (
                  <button key={asset.symbol} onClick={() => handleTokenSelect(asset)} className="w-full flex items-center justify-between p-4 rounded-3xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all text-left">
