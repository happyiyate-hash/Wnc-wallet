...
export default function ProfilePage() {
        ...
        try {
            ...
            const { error: profileError } = await supabase
                ...
                .upsert({
                    ...
                    display_name: profile?.display_name || user.email?.split('@')[0] || 'Institutional User'
                }, { onConflict: 'user_id' });
    ...
    const identity = profile?.account_number;
                ...
                <div className="max-w-2xl mx-auto p-6 space-y-8">
                        ...
                        <div className="space-y-2 flex flex-col items-center w-full">
                            {identity ? (
                                <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 animate-in fade-in zoom-in duration-500">
                                    <span className="text-[11px] font-black text-primary uppercase tracking-[0.1em]">@{identity}</span> 
                                    <button onClick={() => copy(`@${identity}`)} className="p-1 hover:bg-primary/20 rounded-md transition-colors">
                    ...
                    <section className="space-y-3">
                        ...
                        <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/20 space-y-4 shadow-2xl relative group">
                            <div className="flex items-center justify-between">
                                ...
                                <div className={cn("flex items-center gap-1.5 text-[8px] font-black uppercase tracking-widest", isSyncing ? "text-primary animate-pulse" : "text-green-500")}>
                                    <div className={cn("w-1 h-1 rounded-full bg-current", !isSyncing && "animate-pulse")} />
                                    {isSyncing ? "Syncing..." : "Cloud Verified"}
                                </div>
                            </div>
                            
                            <div onClick={() => address && copy(address)} className="bg-black/40 border border-white/10 rounded-2xl p-4 flex items-center justify-between cursor-pointer hover:border-primary/50 transition-all">
                                <p className="text-xs font-mono text-white/70 break-all leading-relaxed mr-4">
                                    {address || 'Initializing Secure Node...'}
                                </p>
                                <div className={cn("shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all", isCopied ? "bg-green-500/20 text-green-500" : "bg-primary/20 text-primary")}>
                    ...
                    <section className="space-y-3">
                        ...
                        <div className="space-y-2">
                            <ProfileAction icon={Store} label="View My Business" value="SmarterSeller Shop" accentColor="text-emerald-400" badge="Live" onClick={() => router.push('/browse')} />
                            <ProfileAction icon={Users} label="Invite & Earn" value="Referral Program" accentColor="text-blue-400" onClick={() => router.push('/tokens/add')} />
                            <ProfileAction icon={Fingerprint} label="Webiv ID Status" value={identity ? `@${identity}` : 'Unclaimed Handle'} accentColor="text-purple-400" onClick={() => !identity && setIsClaimModalOpen(true)} />
                        </div>
                    </section>
                </div>
            </main>
                ...
                <DialogContent className="bg-[#0a0a0c] border-white/10 rounded-[2.5rem] p-8 max-w-[95vw] sm:max-w-[400px] shadow-2xl">
                    <DialogHeader className="space-y-4">
                        ...
                        <DialogDescription className="text-center text-zinc-400 leading-relaxed font-medium">
                            Set a human-readable identifier for your multi-chain vault.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="mt-6 space-y-4">
                        <div className="relative">
                            ...
                            <Input placeholder="username" value={claimHandle} onChange={(e) => setClaimHandle(e.target.value.replace(/[^a-zA-Z0-9_]/g, ''))} className="h-14 bg-white/5 border-white/10 pl-10 rounded-2xl focus-visible:ring-primary text-lg font-bold" />
                        </div>
                        <Button className="w-full h-14 rounded-2xl font-black text-lg bg-primary hover:bg-primary/90 shadow-xl shadow-primary/20" onClick={handleClaim} disabled={isClaiming || claimHandle.length < 3}>
