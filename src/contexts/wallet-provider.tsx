...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  // 1. Resolve Effective Network Node
  ...
  const allAssets = useMemo(() => {
    ...
    const wncAsset = { chainId: effectiveViewingNetwork.chainId, address: 'internal:wnc', symbol: 'WNC', name: 'Wevinacoin', balance: profile?.wnc_earnings?.toString() || '0', isNative: false, priceUsd: wncPrice, fiatValueUsd: (profile?.wnc_earnings || 0) * wncPrice, pctChange24h: prices['internal:wnc']?.change || 0, decimals: 0 } as AssetRow;
    ...
    const onChainAssets = registry.map(asset => {
      ...
      return { ...asset, balance: balDoc?.balance || '0', priceUsd: livePrice, fiatValueUsd: parseFloat(balDoc?.balance || '0') * livePrice, pctChange24h: p?.change || 0 } as AssetRow;
    });

    return [wncAsset, ...onChainAssets].filter(a => !hiddenTokenKeys.has(`${effectiveViewingNetwork.chainId}:${a.symbol}`)).sort((a, b) => (b.fiatValueUsd || 0) - (a.fiatValueUsd || 0));
  ...
  const { refresh } = useWalletEngine({ wallets, viewingNetwork: effectiveViewingNetwork, user, chainsWithLogos, userAddedTokens, infuraApiKey, setBalances: handleSetBalances, setIsRefreshing, setHasFetchedInitialData });
  ...
  const restoreFromCloud = useCallback(async (onStatusUpdate?: (status: string) => void) => {
    ...
    try {
      if (profile.vault_phrase && profile.iv) {
        const res = await fetch('/api/wallet/decrypt-phrase', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ encrypted: profile.vault_phrase, iv: profile.iv }) });
    ...
    } catch (e: any) { throw new Error("Cloud handshake failed."); } finally { setIsWalletLoading(false); }
  }, [user, profile, toast, runCloudDiagnostic]);

  const deleteWalletPermanently = useCallback(async () => {
    if (!user) return;
    try {
      await supabase.from('profiles').update({ vault_phrase: null, iv: null, account_number: null, onboarding_completed: false, evm_address: null, xrp_address: null, polkadot_address: null, kusama_address: null, near_address: null, solana_address: null, btc_address: null, ltc_address: null, doge_address: null, cosmos_address: null, osmosis_address: null, secret_address: null, cardano_address: null, tron_address: null, algorand_address: null, hedera_address: null, tezos_address: null, aptos_address: null, sui_address: null }).eq('id', user.id);
      registryDb.purgeAll(); purgeLocalWalletCache(user.id);
    ...
    const hydrate = async () => {
      try {
        ...
        if (savedMnemonic) {
          ...
          if (cached) setWallets(cached.wallets);
          else { const derived = await deriveAllWallets(savedMnemonic); setWallets(derived); await registryDb.saveVault({ id: fingerprint, wallets: derived, accountNumber: savedAcc || '', timestamp: Date.now() }); }
        }
