...
export default function QuickSwapPanel({ isOpen, onOpenChange }: QuickSwapPanelProps) {
  ...
  return (
    <>
        ...
        >
            <div className="bg-zinc-950/95 backdrop-blur-3xl border border-white/10 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.7)] p-3.5 max-w-lg mx-auto pointer-events-auto relative overflow-hidden group">
                <div className="absolute -top-24 -right-24 w-40 h-40 bg-primary/10 blur-[100px] rounded-full pointer-events-none" />
                
                <div className="flex items-center justify-between mb-3 relative z-10">
                    <div className="flex items-center gap-1.5">
                        <div className="w-1 h-1 rounded-full bg-primary animate-pulse" />
                        <span className="text-[7px] font-black uppercase tracking-[0.3em] text-primary/80">Institutional Quick Exchange</span>
                    </div>
                    <button 
                        onClick={() => onOpenChange(false)}
                        className="p-1 rounded-full hover:bg-white/10 transition-colors bg-white/5 border border-white/5"
                    >
                ...
                <div className="flex items-center justify-between gap-2 mb-3 relative z-10">
                    <button 
                        ...
                        className="flex items-center gap-1.5 hover:bg-white/10 p-1 rounded-xl transition-all active:scale-95 bg-white/5 border border-white/5 group/btn"
                    >
                        <TokenLogoDynamic 
                        ...
                        />
                        <span className="text-[9px] font-black text-white uppercase pr-1">{fromToken?.symbol}</span>
                    </button>

                    <div className="flex-1 flex items-center justify-center relative px-2">
                        <div className="absolute inset-x-0 h-[1px] border-t border-dashed border-white/10" />
                        <div className="w-5 h-5 rounded-full bg-zinc-900 border border-white/10 flex items-center justify-center relative z-10 shadow-lg group-hover:scale-110 transition-transform">
                            <Plane className="w-2.5 h-2.5 text-primary" />
                        </div>
                    </div>

                    <button 
                        onClick={() => { setSelectionType('to'); setIsNetworkSheetOpen(true); }}
                        className="flex items-center gap-1.5 text-right hover:bg-white/10 p-1 rounded-xl transition-all active:scale-95 bg-white/5 border border-white/5 group/btn"
                    >
                        <span className="text-[9px] font-black text-white uppercase pl-1">{toToken?.symbol}</span>
                        <TokenLogoDynamic 
                ...
                <div className="flex items-center justify-between gap-4 px-1 relative z-10">
                    <div className="flex-1 min-w-0">
                        <p className="text-[6px] font-black text-muted-foreground uppercase mb-0.5 tracking-[0.2em] opacity-50">Sell</p>
                        <Input 
                            type="number"
                            placeholder="0.00"
                            value={amount}
                    ...
                    <div className="text-right">
                        <p className="text-[6px] font-black text-muted-foreground uppercase mb-0.5 tracking-[0.2em] opacity-50">Buy (Est)</p>
                        {isQuoteLoading ? (
                            <Skeleton className="h-5 w-20 bg-white/5 rounded-lg ml-auto" />
                        ) : (
                            <div className={cn(
                                "text-lg font-black tracking-tighter transition-all tabular-nums",
                ...
                <div className="flex items-center justify-between text-[6px] font-black uppercase tracking-[0.15em] text-muted-foreground/60 mb-3.5 relative z-10 px-1">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1">
                            <Timer className="w-2.5 h-2.5 text-primary/50" />
                            {isQuoteLoading ? <Skeleton className="h-1.5 w-6 bg-white/5" /> : <span>{executionTimeMin > 0 ? `${executionTimeMin}M` : '--'}</span>}
                        </div>
                        <div className="flex items-center gap-1">
                            <Fuel className="w-2.5 h-2.5 text-primary/50" />
                            {isQuoteLoading ? <Skeleton className="h-1.5 w-10 bg-white/5" /> : <span>${gasCostUsd.toFixed(2)} Fee</span>}
                        </div>
                    </div>
                ...
                <Button 
                    className={cn(
                        "w-full h-10 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] shadow-2xl transition-all border-b-2 border-primary/40 active:border-b-0 active:translate-y-0.5",
        ...
        <Sheet open={isNetworkSheetOpen} onOpenChange={setIsNetworkSheetOpen}>
            <SheetContent side="bottom" className="bg-transparent border-t border-primary/20 rounded-t-[3rem] p-0 h-[80vh] overflow-hidden z-[110]">
                ...
                <div className="flex flex-col h-full relative z-10">
                    ...
                    <SheetHeader className="mb-6 px-6 shrink-0 pt-2">
        ...
        <Sheet open={isTokenSideSheetOpen} onOpenChange={setIsTokenSideSheetOpen}>
            <SheetContent side="right" className="bg-[#050505]/95 backdrop-blur-2xl border-l border-white/5 w-full sm:max-w-[400px] p-0 flex flex-col h-full z-[120]">
